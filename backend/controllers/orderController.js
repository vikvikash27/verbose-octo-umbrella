...
// // Helper function to adjust stock levels for an order's product variations
// const adjustStock = async (order, direction) => {
//   const multiplier = direction === "decrement" ? -1 : 1;
//   for (const item of order.items) {
//     try {
...
//         );
//         if (variation) {
//           variation.stock += multiplier * item.quantity;
//           // Prevent stock from going negative on decrement
//           if (variation.stock < 0) variation.stock = 0;
//           await product.save(); // Triggers pre-save hook to update totalStock and status
//         } else {
//           console.warn(
//             `Variation "${item.variationName}" not found for product "${product.name}". Stock not adjusted.`
//           );
//         }
//       } else {
//         console.warn(
//           `Product with ID ${item.productId} not found. Stock not adjusted.`
//         );
//       }
//     } catch (error) {
//       console.error(
//         `Error adjusting stock for product ${item.productId}:`,
//         error
//       );
//     }
//   }
...
//       address,
//       transactionId:
//         transactionId || `txn_${paymentMethod.toLowerCase()}_${Date.now()}`,
//       statusHistory: [{ status: "Pending", timestamp: now }],
//     });
...
//         if (status === "Processing" && previousStatus === "Pending") {
//           await adjustStock(order, "decrement");
//         } else if (status === "Cancelled" && previousStatus === "Processing") {
//           // Only restock if order was cancelled from a state where stock was already deducted
//           await adjustStock(order, "increment");
//         }
//       }
...
//       return res
//         .status(401)
//         .json({ message: "User not authorized to cancel this order" });
//     }
...
//     // No stock adjustment needed since it was cancelled from 'Pending' state
...
//     const updateResult = await Order.updateMany({ id: { $in: orderIds } }, [
//       {
//         $set: {
//           status: status,
//           statusHistory: {
...
//       "Order",
//       orderIds.join(", "),
//       `Set status to ${status} for ${orderIds.length} orders.`
//     );
...
/////////////////////////////////Inventory Automation Code Ends///////////////////////////////
...
// Helper function to adjust stock levels. Made more robust and efficient.
const adjustStock = async (order, direction) => {
  const multiplier = direction === "decrement" ? -1 : 1;

  // Create an array of promises for all the product updates for this order.
  const updatePromises = order.items.map(async (item) => {
    try {
      ...
      if (product) {
        ...
        if (variation) {
          ...
          if (variation.stock < 0) {
            console.warn(
              `Stock for ${product.name} - ${variation.name} would go negative. Setting to 0.`
            );
            variation.stock = 0;
          }

          // Explicitly tell Mongoose that the 'variations' array has been modified.
          // This is crucial for change detection within nested arrays of objects and fixes the bug.
      ...
      // We don't re-throw here to allow other items in the order to be processed.
    }
  });

  // Wait for all product updates for this order to complete.
...
const bulkUpdateOrderStatus = async (req, res) => {
  try {
    ...
    // Collect all stock adjustment promises to run them in parallel
    const stockAdjustmentPromises = [];
    for (const order of ordersToUpdate) {
      const previousStatus = order.status;
      if (previousStatus !== status) {
        if (status === "Processing" && previousStatus === "Pending") {
          stockAdjustmentPromises.push(adjustStock(order, "decrement"));
        } else if (status === "Cancelled" && previousStatus === "Processing") {
          stockAdjustmentPromises.push(adjustStock(order, "increment"));
        }
      }
    }

    // Execute all stock adjustments concurrently for better performance
